...
def update(presentation,photos,numbersH_free,numbersV_free,slide):
...
def algo_greedy(instance,temps_limite = 100000,greedy=2):
    ...
    tags1 = update(presentation,photos,numbersH_free,numbersV_free,slide1)
    ...
    while(time.time()-debut < temps_limite):
        ...
        #put the photos in the form of a slide/list
            ...
            if photo2 != None:
                ...
                slide2 = [photo1]
        #update the presentation, free numbers and the current tag
        tags1 = update(presentation,photos,numbersH_free,numbersV_free,slide2)
        if(len(presentation)%100 == 0):
            print("{} slides in {} secs".format(len(presentation),time.time()-debut))
    return presentation

############################## Stochastic descent #############################
def score_neighbor(pres,photos,ind1,ind2):
...
def algo_descent(pres,photos,max_iter = None):
    ...
    for i in range(max_iter):
        ...
        if(r<p):
            ...
            score = score_neighbor(pres,photos,ind1,ind2)
            pres[ind1],pres[ind2] = pres[ind2],pres[ind1]
            score_desc = score_neighbor(pres,photos,ind1,ind2)
...
def LP(photos,integer = False,sub_tours = 0):
    """ linear programming
...
def rounding(solution_lp,photos):
    ...
    while len(presentation_lp) != n:
        ...
        index = chain[index]
        presentation_lp.append([index])
    return presentation_lp
